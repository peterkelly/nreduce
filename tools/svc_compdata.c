...
void handle(int sock)
{
  ...
  int ms;
  if (1 == sscanf(data,"%d",&ms)) {
    /* Compute for requested time */
    run(comp_per_ms*ms);

    /* Send back data supplied by client */
    write(sock,data,size);

    printf("Done call, ms = %d, data size = %d\n",ms,size);
  }
  else if (1 == (r = sscanf(data,"process %d %n",&ms,&datapos))) {
    run(comp_per_ms*ms);
    write(sock,&data[datapos],size-datapos);
    printf("Done process request, ms = %d, data = %d bytes\n",ms,size-datapos);
  }
  else if (1 == (r = sscanf(data,"combine %d %n",&ms,&datapos))) {
    run(comp_per_ms*ms);
    int itemsize = -1;
    int curlen = 0;
    int nitems = 0;
    int pos;
    for (pos = datapos; pos <= size; pos++) {
      if ((pos == size) || (' ' == data[pos])) {
        if (-1 == itemsize) {
          itemsize = curlen;
        }
        else if (curlen != itemsize) {
          printf("ERROR: Inconsistent item lengths: got %d, expected %d\n",
                 curlen,itemsize);
          exit(1);
        }
        curlen = 0;
    ...
    if (itemsize > 0) {
      write(sock,&data[datapos],itemsize);
    }
