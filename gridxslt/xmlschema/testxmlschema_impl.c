...
int testxmlschema_main(int argc, char **argv)
{
  ...
  if (arguments.dump)
    ...
    r = print_hierarchy(stdout,s);
  else if (arguments.types)
    r = dump_all_types(&di,s);
  else if (arguments.attributes)
    r = dump_all_attributes(&di,s);
  else if (arguments.elements)
    r = dump_all_elements(&di,s);
  else if (arguments.attribute_groups)
    r = dump_all_attribute_groups(&di,s);
  else if (arguments.identity_constraints)
    r = dump_all_identity_constraints(&di,s);
  else if (arguments.model_groups)
    r = dump_all_model_groups(&di,s);
  else if (arguments.notations)
    r = dump_all_notations(&di,s);
  else if (arguments.wildcards)
    r = dump_all_wildcards(&di,s);
  else if (arguments.wildcard_union)
    r = dump_wildcard_union(&di,s,arguments.typenames);
  else if (arguments.wildcard_intersection)
    r = dump_wildcard_intersection(&di,s,arguments.typenames);
  else if (arguments.wildcard_subset)
    r = dump_wildcard_subset(&di,s,arguments.typenames);
  else if (arguments.range)
    r = print_range(s,arguments.typenames);
  else if (arguments.particle_derivation)
    r = test_particle_derivation(s,arguments.typenames,arguments.final_set);
    ...
    r = print_non_pointless(s,arguments.typenames);
  else
    output_xmlschema(stdout,s);
